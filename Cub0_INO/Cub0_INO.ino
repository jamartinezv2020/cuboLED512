...
uint8_t abc[26][8] = {
  ...
  {0x1E, 0x22, 0x22, 0x22, 0x1E, 0x12, 0x22, 0x42}, // R 
  {0x3C, 0x02, 0x02, 0x1C, 0x20, 0x20, 0x20, 0x1E}, // S 
  ...
  {0x91, 0x91, 0x91, 0xAA, 0xAA, 0xAA, 0x44, 0x44}, // W 
...
bool showingNumbers = false;  // Controla si se están mostrando números
bool showingAlphabet = false; // Controla si se está mostrando el abecedario
...
//   if (digitalRead(BUTTON_PIN_A) == LOW || (currentTime - lastEffectChange >= effectDuration && autoRotate)) {
...
//     // Detecta cuando se presiona el botón B para alternar entre números y abecedario
//   if (digitalRead(BUTTON_PIN_B) == LOW) {
//     if (!showingNumbers && !showingAlphabet) {
//       showingNumbers = true;
...
//   // Muestra los números si está activado
//   if (showingNumbers) {
//     showNumbers();
//   }

//   // Muestra el abecedario si está activado
//   if (showingAlphabet) {
//     showAlphabet();
//   }

//   // Si no se están mostrando números ni abecedario, ejecuta los efectos regulares
//   if (!showingNumbers && !showingAlphabet) {
//     switch (currentEffect) {
//       case RAIN: rain(); break;
//       case PLANE_BOING: planeBoing(); break;
...
//   renderCube(); // Renderiza el cubo en cada ciclo
// }
...
void loop() {
  ...
  // Detecta cuando se presiona el botón A para cambiar de efecto
  if (digitalRead(BUTTON_PIN_A) == LOW || (currentTime - lastEffectChange >= effectDuration && autoRotate)) {
    lastEffectChange = millis();
    clearCube();
    loading = true;
...
void showNumbers() {
  for (int i = 0; i < 10; i++) {
    displayCharacter(number[i]);
    delay(TEXT_TIME); // Tiempo entre cada número
  }
}

void showAlphabet() {
  for (int i = 0; i < 26; i++) {
    displayCharacter(abc[i]);
    delay(TEXT_TIME); // Tiempo entre cada letra
  }
}
...
void displayCharacter(uint8_t character[8]) {
  clearCube();
  for (uint8_t i = 0; i < 8; i++) {
    for (uint8_t j = 0; j < 8; j++) {
      if (character[i] & (1 << j)) {
        setVoxel(j, 0, i); // Ajustar la posición según tu orientación
      }
    }
  }
  renderCube();
